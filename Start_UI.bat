...
if not exist "%REPO_ROOT%\.venv\Scripts\activate.bat" (
  if exist "%REPO_ROOT%\scripts\one_click_install_and_run.bat" (
    call "%REPO_ROOT%\scripts\one_click_install_and_run.bat"
  )
)
if not exist "%REPO_ROOT%\.venv\Scripts\activate.bat" (
  echo [!] Virtualenv not found; please run scripts\gigatrader_menu.bat option 1 (Install/Update).
  pause & exit /b 1
)
