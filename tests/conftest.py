...
# Always load asyncio plugin even when autoload is disabled
...
def pytest_sessionstart(session):
    ...
    # Clean only repo-level permanent switch if present
...
def _default_disarm_kill_switch(monkeypatch, tmp_path):
    # Defense-in-depth for tests that mutate env mid-run
