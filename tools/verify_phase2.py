...
def grep_tokens():
    ...
    for t in ["class Gateway", "propose_order", "RiskManager", "Proposal", "Decision"]:
        if t not in g:
            fail(f"gateway/proposals.py missing: {t}")
    if FAIL == 0:
        ok("Symbols present in Phase-2 modules")
...
def import_sanity():
    ...
    for cls in ["RiskManager", "Proposal", "Decision"]:
        if not hasattr(eng, cls):
            fail(f"engine missing {cls}")
